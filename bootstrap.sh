...
sudo cp /vagrant/confluent.repo /etc/yum.repos.d/
...
sudo JMX_PORT=9990 LOG_DIR=/var/log/kafka0 kafka-server-start /vagrant/server0.properties &
sudo JMX_PORT=9991 LOG_DIR=/var/log/kafka1 kafka-server-start /vagrant/server1.properties &
sudo JMX_PORT=9992 LOG_DIR=/var/log/kafka2 kafka-server-start /vagrant/server2.properties &
sudo schema-registry-start /etc/schema-registry/schema-registry.properties &
sudo kafka-rest-start /etc/kafka-rest/kafka-rest.properties &
...
sudo gunicorn_django --bind=0.0.0.0:8000 --log-file=/var/log/gunicorn-graphite/gunicorn.log --preload --pythonpath=/usr/lib/python2.6/site-packages/graphite --settings=settings --workers=3 --pid=/var/run/gunicorn-graphite/gunicorn-graphite.pid &

wget http://central.maven.org/maven2/org/jmxtrans/jmxtrans/251/jmxtrans-251.rpm
sudo yum -y install jmxtrans-251.rpm
yaml2jmxtrans /vagrant/kafka.yaml
